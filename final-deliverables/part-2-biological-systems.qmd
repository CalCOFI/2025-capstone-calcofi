...
[^1]: Bottle Database: <https://calcofi.org/data/oceanographic-data/bottle-database/>
...
## CalCOFI NOAA Zooplankton Volume[^2]:

[^2]: CalCOFI Zooplankton Volume Database: <https://oceanview.pfeg.noaa.gov/erddap/tabledap/erdCalCOFIzoovol.html>
...
## BTEDB (Krill) Abundances[^3]:

[^3]: BTEDB (Krill Volume) Data: <https://portal.edirepository.org/nis/mapbrowse?packageid=knb-lter-cce.313.1>
...
## PRPOOS (Calcifiers/Non-Calcifiers)[^4]:

[^4]: PRPOOS (Zooplankton Calcifiers/Non-Calcifiers Volume) Zooscan Database: <https://oceaninformatics.ucsd.edu/zooscandb/>
...
Construct GAM models to assess the effect of OmegaARout across different quarters. Iterate through Nearshore and Offshore datasets
...
### PRPOOS Calcifying vs Non-calcifying

Calculate the calcifying_ratio as the ratio of calcifying to non-calcifying species. Classify observations as High or Low based on the median calcifying_ratio. Build a logistic regression model to assess the impact of pH_mean, OmegaAR_mean, and CO3_mean.
...
if (length(unique(prpoos$calcifying_class)) > 1) {
  logistic_model <- glm(
    calcifying_class ~ pH_mean * OmegaAR_mean + CO3_mean,
    data = prpoos,
    family = binomial
  )
  #write.csv(coef(summary(logistic_model)), "logistic_model_summary.csv")
} else {
  cat("Warning: calcifying_class has only one level. Skipping logistic regression.\n")
}
...
prpoos_summary$cluster <- kmeans_model$cluster

cluster_means <- prpoos_summary %>%
  group_by(cluster) %>%
  summarize(
...
#write.csv(cluster_means, "cluster_means_detailed.csv")

fviz_pca_ind(pca_result, label = "none", habillage = prpoos_summary$cluster, addEllipses = TRUE, ellipse.level = 0.95) +
