...
![Table 2: Hypothesis testing results for the mean of the ESPER residuals (the bias, as described in Equation 1). The displayed p-values have been adjusted for alpha inflation using the Benjamini-Hochberg procedure.](~/capstone-calcofi-seagrant/interim-report/img/team6-table2.png){width=50%}
...
![Figure 1: ESPER NN predictions plotted against observed values of TA and DIC. Perfect predictions are represented by the solid diagonal line in black (the further a point is from this line, the less accurate the prediction).](~/capstone-calcofi-seagrant/interim-report/img/team6-figure1.png){width=80%}
...
![Table 3: Mixed effects regression results for hydrography and carbonate chemistry variables regressed against time and depth. Shown values refer to the estimated parameter for time.](~/capstone-calcofi-seagrant/interim-report/img/team6-table3.png){width=70%}
...
![Figure 3: Map of aggregated model results by station.](~/capstone-calcofi-seagrant/interim-report/img/team6-figure3.png){width=80%}
...
We created two spatial GAM models for log-transformed total plankton abundance and log-transformed small plankton abundance, respectively. Both response variables use the smooth parameters pH, TA, DIC, salinity, temperature, year, month, and station ID and tensor product smooths between latitude and longitude.

#### Total Plankton Abundance


![Figure 4: Spatial smooth from the GAM model showing the partial effect of latitude and longitude on log-transformed total plankton abundance. Warmer colors indicate regions associated with lower abundance, while cooler yellow tones represent areas with higher predicted abundance.](~/capstone-calcofi-seagrant/interim-report/img/team6-figure4.png){width=70%}
...
The GAM model reveals that spatial location, seasonality (month), and long-term temporal trends (year) are the strongest predictors of total plankton abundance, explaining 28% of the variability. Figure 4 shows higher abundances are predicted along the eastern and southeastern coastal regions, while lower abundances appear offshore, especially in the southwestern region. From the model results, pH showed a slight positive relationship with abundance while other variables like TA, DIC, salinity, and temperature did not have significant effects in this model. 


#### Small Plankton Abundance

![Figure 6: Spatial smooth from the GAM model showing the partial effect of latitude and longitude on log-transformed small plankton abundance.](~/capstone-calcofi-seagrant/interim-report/img/team6-figure6.png){width=50%}
...
The GAM model explains approximately 35% of the variance in log-transformed small plankton abundance, with significant effects from spatial location, pH, TA, temperature, year, and month. Figure 6 shows higher abundance predicted along the southern and eastern coastal regions, and lower abundance offshore to the northwest. Figure 7 shows that plankton abundance tends to increase with higher pH, suggesting potential sensitivity of small plankton to acidification. Higher TA is associated with decreased abundance, while temperature has a nonlinear effect. From the model summary, other variables such as DIC, salinity, and station ID showed no significant influence on abundance.
...
![Figure 8: Random Forest model confusion matrix and call output for presence/absence classification of Thysanoessa gregaria.](~/capstone-calcofi-seagrant/interim-report/img/team6-figure8.png){width=60%}
...
Notably, the model achieved 100% accuracy for predicting presence ('yes'), correctly classifying all 41 presence cases. The confusion matrix indicates most misclassifications occurred in the absence category.

![Figure 9: Variable importance plots based on Mean Decrease Accuracy and Gini Index from the Random Forest model.](~/capstone-calcofi-seagrant/interim-report/img/team6-figure9.png){width=60%}

Feature importance analysis in Figure 9 revealed that Station_ID was the most influential variable by a wide margin, suggesting strong spatial dependence. This implies the model effectively "memorized" geographic patterns in T. gregaria distribution, highlighting that occurrence is highly dependent on location.
...
::: {layout-ncol=2}
![Figure 10: Elbow plot showing the total within-cluster sum of squares for k = 1 to 10.](~/capstone-calcofi-seagrant/interim-report/img/team6-figure10.png){width=70%}

 

![Figure 11: PCA biplot overlaid with K-means clustering results (k = 3).](~/capstone-calcofi-seagrant/interim-report/img/team6-figure11.png){width=70%}
:::


Looking at the elbow plot in Figure 10, a sharp decline is observed up to k = 3, suggesting three clusters as the optimal choice for grouping ecological sample points. In Figure 11, each cluster corresponds to a distinct grouping in environmental space, indicating different habitat types for T. gregaria.

Following PCA, K-means clustering was applied to the first two principal components. The optimal number of clusters (k) was determined using the elbow method, which plots the total within-cluster sum of squares against increasing k values. 

As shown in the elbow plot (Figure 10), a clear inflection point at k = 3 indicated that three clusters sufficiently captured the major structure of the data without overfitting.  The K-means clustering divided the samples into three distinct groups:  Cluster 1 included observations with the highest mean log-transformed abundance (3.10), corresponding to favorable environmental conditions for T. gregaria. Cluster 2 represented sites with moderate mean abundance (2.58), likely reflecting transitional or marginal habitats. Cluster 3 contained all observations with zero abundance, indicating environmentally unsuitable sites.

![Table 4: Mean log-transformed abundance of Thysanoessa gregaria for each of the three clusters.](~/capstone-calcofi-seagrant/interim-report/img/team6-table4.png){width=60%}

From Table 4, Cluster 1 has the highest mean abundance, while Cluster 3 contains only zero values, suggesting unsuitable habitat or timing. Each cluster reflected distinct habitat characteristics:  
- Cluster 1: High mean log-abundance (3.10) — likely represents ideal environmental conditions. 
- Cluster 2: Intermediate abundance (2.58) — may correspond to marginal habitats or transitional seasonal conditions. 
- Cluster 3: Zero abundance — likely unsuitable sites or times for T. gregaria presence.
...
::: {layout-ncol=2}
![Table 5: OLS model results of the abundance of calcifying species using predictors selected by LASSO regression.](~/capstone-calcofi-seagrant/interim-report/img/team6-table5.png)

![Table 6: OLS model results of the abundance of other species using predictors selected by LASSO regression. ](~/capstone-calcofi-seagrant/interim-report/img/team6-table6.png)
:::
...
![Figure 12: GAM model with spatial splines for each calcifying species.](~/capstone-calcofi-seagrant/interim-report/img/team6-figure12.png)

![Figure 13: GAM model with spatial splines for other species (first 6 species are shown).](~/capstone-calcofi-seagrant/interim-report/img/team6-figure13.png)
