...
cc_bottle <- cc_bottle %>% mutate(DIC = as.numeric(DIC),
                                  TA = as.numeric(TA),
...
obs_count <-  cc_bottle %>% count(Year_UTC, Month_UTC, Station_ID) 
...
num_cc<- cc_bottle[,c("Date", "Year_UTC", "Month_UTC", "Day_UTC", "Station_ID", 
                      "Latitude", "Longitude", "TA", "DIC", "Depth", 
                      "CTDTEMP_ITS90", "Salinity_PSS78")]
cc_avg_daily <- num_cc %>% group_by(Date, Station_ID) %>% 
...
# average the cc_bottle data per month per location
num_cc<- cc_bottle[,c("Date", "Year_UTC", "Month_UTC", "Day_UTC", "Station_ID", 
                      "Latitude", "Longitude", "TA", "DIC", "Depth", 
                      "CTDTEMP_ITS90", "Salinity_PSS78")]
