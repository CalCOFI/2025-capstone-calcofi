...
multi_seq_bottle_co2sys <- multi_seq_bottle_co2sys |> filter(Station_ID != "080.0 100.0") |> 
  filter(Station_ID != "080.0 090.0")

keep_stations <- multi_seq_bottle_co2sys %>%
  group_by(
    Station_ID
  ) %>%
  summarize(
  ...
  setdiff(
    x = unique(multi_seq_bottle_co2sys$Station_ID)
  )
multi_seq_bottle_co2sys <- multi_seq_bottle_co2sys %>%
  filter(
...
multi_seq_bottle_co2sys1 <- multi_seq_bottle_co2sys |> filter(Depth < 1000)
pCO2in_mod1 <- lme(
...
lapply(
  ...
  function(i){
    ggplot() + 
      geom_point(aes(y = resid(models[[i]]), x= fitted(models[[i]]))) +
      labs(x= paste0("fitted ", qty[i])) +
      labs(y = paste0(qty[i], " residuals"))
  })
...
pCo2_bayes <- stan_glmer(formula = pCO2in ~ Date_Dec + Depth + (Date_Dec | Station_ID),
                  data = multi_seq_bottle_co2sys,
                  family = "gaussian",
