...
```
1. 按总丰度或出现频率筛选
总丰度：计算每个物种在整个数据集中的丰度总和，选取丰度最高的若干个物种（比如前 5 或前 10）。
出现频率：计算每个物种非零（>0）丰度在所有采样中的出现比例（presence frequency），若出现过于稀少，就可能缺乏代表性。
```{r}
library(tidyverse)
...
```{r}
ggplot(df_long, aes(x = CTDTEMP_ITS90, y = Salinity_PSS78)) +
  geom_point(aes(size = Abundance, color = Depth), alpha = 0.6) +
  facet_wrap(~ Species) +
  labs(
    title = "T-S Diagram by Species (Point Color ~ Depth, Size ~ Abundance)",
    x = "Temperature (°C)",
    y = "Salinity (PSS78)",
    size = "Abundance",
    color = "Depth"
...
```{r}
ggplot(df_long, aes(x = CTDTEMP_ITS90, y = Salinity_PSS78)) +
  geom_point(aes(color = Stage, size = Abundance), alpha = 0.6) +
  facet_wrap(~ Species) +
  labs(
    title = "T-S Distribution (Colored by Stage, Sized by Abundance)",
    x = "Temperature (°C)",
    y = "Salinity (PSS78)"
  ) +
  theme_minimal()

ggsave(file.path("JJF Krill Plots (Analysis)", "T-S Distribution(Stage, Abundance).png"))
```
