...
lon_seq <- seq(min(prpoos_summary$Longitude) - 1,
               max(prpoos_summary$Longitude) + 1,
               length.out = 200)
lat_seq <- seq(min(prpoos_summary$Latitude) - 1,
               max(prpoos_summary$Latitude) + 1,
               length.out = 200)
...
# Predict from GAM
grid$fit <- predict(gam_model, newdata = grid)
...
ggplot() +
  ...
  coord_quickmap(xlim = range(prpoos_summary$Longitude) + c(-0.5, 0.5),
                 ylim = range(prpoos_summary$Latitude) + c(-0.5, 0.5)) +
