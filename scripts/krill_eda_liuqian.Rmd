...
plot_saturation_horizon <- function(station_id, data) {
  ...
  for (species in years) {
    ...
    if (nrow(filtered_data) > 1 && n_unique > 2) {  
      ...
      fit_bs <- lm(Depthm ~ bs(OmegaARout, degree = 1, knots = knotpts),
                   data = filtered_data) 
      # Base R plot
      plot(filtered_data$OmegaARout, filtered_data$Depthm, pch = 16, xlab = "OmegaARout", ylab = "Depth (m)", main = "Spline Fit")

      # Add fitted line
      ord <- order(filtered_data$OmegaARout)
      lines(filtered_data$OmegaARout[ord], predict(fit_bs)[ord], col = "blue", lwd = 2)

      
      predicted_depth <- predict(fit_bs, newdata = tibble(OmegaARout = 1))
