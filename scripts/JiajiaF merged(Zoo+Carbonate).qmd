...
oah_bottle <- oah_bottle %>%
  mutate(datetime_UTC = make_datetime(Year_UTC, Month_UTC, Day_UTC, 
...
colnames(new_zooplankton)
new_zooplankton <- new_zooplankton %>%
  mutate(Station_ID = paste0("0", line, "0", station)) %>%
  select(-line, -station) # 移除原始列
