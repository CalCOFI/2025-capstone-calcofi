...
# Segment data into two time periods
zooplankton_1 <- zooplankton_clean %>% filter(Year_UTC >= 2010 & Year_UTC <= 2015)
zooplankton_2 <- zooplankton_clean %>% filter(Year_UTC >= 2016 & Year_UTC <= 2021)

# GAM for 2010-2015
gam_2010_2015 <- gam(
  log_total_plankton ~ te(Longitude, Latitude) + s(OmegaARout),
  data = zooplankton_1
)
...
# Compare models
summary(gam_2010_2015)
summary(gam_2016_2021)

# Plot comparison
par(mfrow = c(2, 1))
plot(gam_2010_2015, select = 2, shade = TRUE, main = "2010-2015")
plot(gam_2016_2021, select = 2, shade = TRUE, main = "2016-2021")
par(mfrow = c(1, 1))

# Save the plot
ggsave("time_segment_comparison.png")
...
prpoos <- prpoos %>%
  ...
  mutate(
    ...
    calcifying_ratio = calcifying_abundance / (non_calcifying_abundance + 1),  # Adding 1 to avoid division by zero
    log_calcifying_ratio = log1p(calcifying_ratio)
  )

# GAM model for calcifying ratio
gam_calc_ratio <- gam(
  log_calcifying_ratio ~ s(pH_mean) + s(OmegaAR_mean) + s(CO3_mean),
  data = prpoos
)

# Model summary
summary(gam_calc_ratio)

# Plot
par(mfrow = c(2, 2))
plot(gam_calc_ratio, shade = TRUE)
par(mfrow = c(1, 1))

ggsave("calcifying_ratio_gam.png")
...
dist_matrix <- dist(species_matrix)

# Hierarchical clustering
hclust_model <- hclust(dist_matrix, method = "ward.D2")

# Plot dendrogram
plot(hclust_model, main = "Hierarchical Clustering of Species Abundance")

# K-means clustering
set.seed(123)
kmeans_model <- kmeans(species_matrix, centers = 3)

# Visualize clusters
fviz_cluster(list(data = species_matrix, cluster = kmeans_model$cluster))

ggsave("species_clustering.png")
